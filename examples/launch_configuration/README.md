# Auto Scaling Group with Launch Configuration Example

Configuration in this directory creates autoscaling groups using launch configurations.

There are four different setups shown in this example project:
- `disabled` - demonstrates how to completely disable creating of both the autoscaling group and launch configuration (no resources will be created)
- `default` - demonstrates the bare minimum amount of information required to create an autoscaling group from a launch configuration, relying mostly on default values
- `external` - demonstrates how to create an autoscaling group using an externally created launch configuration
- `complete` - demonstrates the vast majority of the functionality available for creating an autoscaling group using a launch configuration
